...
namespace
{
			...
			)
	{
		...
		for (const Atrap_base* ExistingTrap : ExistingTraps)
		{
			...
			if (FVector::DistSquared2D(SpawnLocation,
			                           ExistingTrap->GetInitialSpawnLocation())
				<= ExclusionDistanceSquared)
			{
				return true;
			}
		}
...
TArray<Atrap_base*> Utrap_spawn::SpawnTrap(
		AActor* OriginActor,
		TSubclassOf<Atrap_base> TrapClass,
		// TrapType TrapType,
		// // const TArray<Atrap_base*>& ExistingTraps,
		...
		)
{
	...
	if (!OriginActor || !TrapClass || SlotCount <= 0 || SpawnCount <= 0)
	{
		return SpawnedTraps;
	}
	...
	const float XStep = SlotCount > 1
		                    ? (XEnd - XStart) / static_cast<float>(SlotCount -
			                    1)
		                    : 0.0f;
